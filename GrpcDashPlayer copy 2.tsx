...
export default function GrpcDashPlayer({ videoId }: { videoId: string }) {
  ...
  const initializePlayer = useCallback(() => {
    ...
    calculateEstimatedBufferEndWrapper,
    isPausedRef,
    videoFinishedRef, // NEW
    audioFinishedRef, // NEW
  ]);
  ...
  useEffect(() => {
    if (videoReps.length > 0) {
      const qualities = [...videoReps]
        ...
        .map((rep, index) => ({
          id: index.toString(), // Use index as string ID
          label: rep.height
            ? `${rep.height}p`
      ...
      const qualityLabel =
        currentQuality === "auto"
          ? "Auto"
          : qualities[Number(currentQuality)]?.label || "Auto";
  ...
  }, [videoReps, currentQuality, setAvailableQualities, setCurrentStats]);
  // useEffect(() => {
