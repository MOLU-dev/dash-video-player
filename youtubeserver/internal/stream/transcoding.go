...
func (m *Manager) StartFFmpegTranscoding(ctx context.Context, streamID, outputDir string, conn *rtmp.Conn) {
	...
	// Add DASH output configuration with custom naming pattern
	args = append(args,
		"-f", "dash",
		"-seg_duration", "2",
		"-use_template", "1",
		...
		// Custom segment naming pattern to match the MPD structure
		"-init_seg_name", "video_$RepresentationID$_dashinit.mp4",
		"-media_seg_name", "video_$RepresentationID$_dash$Number$.m4s",
