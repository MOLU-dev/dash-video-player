...
func (s *Server) CreateStream(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("🎬 Creating new stream: Title='%s', ID=%s, OneTime=%v", req.Title, streamID)  //req.OneTime

	// FIXED: Use proper database parameters
	...
	log.Printf("✅ Stream created successfully in database: %s (OneTime: %v)", streamID) //req.OneTime
}

func (s *Server) CreateOneTimeStream(w http.ResponseWriter, r *http.Request) {
	ctx := context.Background()
	log.Printf("📥 CreateOneTimeStream Request from %s", r.RemoteAddr)
	...
	log.Printf("🔐 Creating ONE-TIME stream: Title='%s', ID=%s", req.Title, streamID)

	// FIXED: Use proper database parameters for one-time stream
	err := s.manager.GetQueries().CreateStream(ctx, db.CreateStreamParams{
		ID:        streamID,
		Title:     sql.NullString{String: req.Title, Valid: req.Title != ""},
		StreamKey: sql.NullString{String: streamKey, Valid: true},
	...
	if err != nil {
		log.Printf("❌ Database error creating one-time stream: %v", err)
		http.Error(w, "Database error", http.StatusInternalServerError)
		return
	}
	...
	response := models.CreateStreamResponse{
		...
		RTMPURL:      fmt.Sprintf("rtmp://localhost:1935/%s", streamKey),
		PlayURL:      fmt.Sprintf("http://localhost:8080/streams/%s/manifest.mpd", streamID),
		Instructions: "⚠️ This stream key can only be used ONCE. After streaming ends, this key will be invalid.",
		//OneTime:      true,
	}

	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(response)
	log.Printf("✅ One-time stream created successfully in database: %s", streamID)
}
...
func (s *Server) GetStream(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("❌ Stream not found: %s, error: %v", streamID, err)
...
func (s *Server) ListStreams(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("✅ Listed %d streams from database", len(streams))
}
...
func (s *Server) GetStreamHealth(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("❌ Stream not found for health check: %s, error: %v", streamID, err)
		http.Error(w, "Stream not found", http.StatusNotFound)
		return
	}
