...
export function useGrpcPlayer({
  ...
  const {
    ...
    videoId,
    metadata: manifestMetadataRef.current,
    videoRepRef,
    videoNextSegRef,
    audioNextSegRef,
    onManifestUpdate: () => {
      const metadata = manifestMetadataRef.current;
      if (metadata) {
    ...
    const handleSeekToLive = useCallback(() => {
      ...
      console.log(`[LIVE] Seeking to live edge: ${liveEdge}s`);
      videoRef.current.currentTime = liveEdge;
    }, [isLive, liveEdge, videoRef]);
   ...
   useEffect(() => {
     ...
     const interval = setInterval(() => {
       const metadata = manifestMetadataRef.current;
       if (metadata?.availabilityStartTime && videoRepRef.current) {
         const now = Date.now();
         const startTime = metadata.availabilityStartTime.getTime();
         const elapsed = (now - startTime) / 1000;
         const delay = metadata.suggestedPresentationDelay || 3;
         const edge = Math.max(0, elapsed - delay);
         setLiveEdge(edge);
       }
     }, 1000);

     return () => clearInterval(interval);
   }, [isLive, manifestMetadataRef, videoRepRef]);
  ...
  // Player initializer
  // wrap switchQuality to satisfy initializer's expected Promise<void> signature
  const switchQualityVoid = useCallback(
    async (newIdx: number) => {
      // preserve original return value but ensure a void Promise is returned
 ...
 const handleSeek = useCallback(
   (time: number) => {
     if (isLive) {
       console.warn("[LIVE] Seeking is not available for live streams");
       return;
     }
  ...
  // Download handlers - Memoized
  const handleDownload = useCallback(() => {
    if (isLive) {
      console.warn("[LIVE] Download is not available for live streams");
      return;
    }
  ...
  }, [videoId, getHighestQualityRep, downloadVideo, durationRef, isLive]);
  ...
  //  handleSeek,
    ...
    liveEdge,
    handleSeekToLive,
    handleSeek, // Updated version
    onDownload: handleDownload, // Updated version
    // Disable resume prompt for live streams
    showResumePrompt: isLive ? false : actualShowResumePrompt,
  };
}
