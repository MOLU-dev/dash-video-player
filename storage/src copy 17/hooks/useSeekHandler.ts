...
import type { MediaType, Representation } from "@/types/player.types";

import { getSegmentNumber } from "@/utils/playerHelpers";

import { removeBufferRange, appendBufferSafely } from "@/utils/bufferHelpers";
import type { ManifestMetadata } from "@/types/player.types";
...
export function useSeekHandler({
  ...
  const resetStreamForSeek = useCallback(
    async (time: number) => {
      ...
      if (manifestMetadataRef.current?.isLive) {
        console.log("[LIVE] Seek disabled for live stream");
        return;
      }
      ...
      if (Date.now() - lastSeekTimeRef.current < 300) {
        console.log("⏸Seek debounced (too soon)");
        return;
      }

      console.log(`🎯 SEEK REQUESTED to ${time.toFixed(2)}s`);
      ...
      try {
        ...
        console.log(
          `DECISION: ${shouldPreserve ? "PRESERVE BUFFER" : "CLEAR BUFFER"}`
        );
