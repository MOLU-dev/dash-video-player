...
export function usePlayerEffects({
  ...
  useEffect(() => {
    ...
    if (saved && saved.currentTime > 0) {
      ...
      // Auto-resume when video is ready and playback starts
      const handleCanPlay = () => {
        if (hasPlaybackStarted && videoRef.current) {
          const resumeTime = Math.max(0, saved.currentTime - 5); // 5 second buffer
          videoRef.current.currentTime = resumeTime;

          // Show toast after video starts playing
          const playHandler = () => {
            setShowResumeToast(true);
  ...
  // Add a timeout to auto-hide the toast if user doesn't interact
  useEffect(() => {
    if (setShowResumeToast) {
      const timer = setTimeout(() => {
  ...
  const handleStartFromBeginning = useCallback(() => {
    if (videoRef.current) {
      ...
      // Ensure video plays after seeking to beginning
      setTimeout(() => {
        videoRef.current?.play().catch(console.error);
      }, 100);
    }
  }, [videoId, clearPosition, setSavedPosition, videoRef]);
