...
export function useReelPlayer({
  ...
  // Play/pause based on active state - FIXED VERSION
  useEffect(() => {
    const video = player.videoRef.current;
    if (!video) return;
    ...
    const handlePlayPause = async () => {
      if (isActive) {
        // Play when active only if video is paused
        if (video.paused && isMounted.current) {
          try {
            await video.play();
        ...
        // Pause when not active - always pause regardless of current state
        if (!video.paused && isMounted.current) {
          video.pause();
        ...
        if (!shouldPreload && video.currentTime > 0 && isMounted.current) {
          video.currentTime = 0;
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const handleCanPlay = () => {
      if (isMounted.current && !hasNotifiedReady.current && onReady) {
  ...
  const playerHandle: ReelPlayerHandle = {
    play: async () => {
      const video = player.videoRef.current;
      if (video && video.paused && isMounted.current) {
        await video.play();
        isPlayingRef.current = true;
      }
    },
