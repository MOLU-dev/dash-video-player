...
export function useReelManager({
  ...
  useEffect(() => {
    return () => {
      // Cleanup all players when component unmounts
      reelRefs.current.forEach((player, reelId) => {
        player.cleanup();
      });
      reelRefs.current.clear();
      console.log("Cleaned up all reel players");
    };
  }, []);
  ...
  // Move to next reel - FIXED VERSION
  const goToNext = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (nextPlayer?.isReady() && !nextPlayer.isPlaying()) {
  ...
  // Move to previous reel - FIXED VERSION
  const goToPrevious = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (prevPlayer?.isReady() && !prevPlayer.isPlaying()) {
  ...
  return {
    ...
    reelRefs, // Export for emergency cleanup if needed
  };
}
