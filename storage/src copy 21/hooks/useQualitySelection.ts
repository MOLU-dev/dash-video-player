...
export function useQualitySelection({
  ...
  const shouldAllowQualitySwitch = useCallback(
    (context: string = "general"): boolean => {
      ...
      if (isPausedRef.current && !isInEmergencyModeRef.current) {
        //console.log("Quality switch blocked - video paused");
        return false;
      }
  ...
  const switchQuality = useCallback(
    async (newIdx: number): Promise<boolean> => {
      const videoEl = videoRef.current;
      if (!videoEl || !videoSbRef.current) {
        console.log("Quality switch failed: No video element or source buffer");
        return false;
      }

      // ✅ CHECK FIRST - Don't update UI if switch won't happen
      ...
      if (!currentRep || !newRep) {
        console.log("Quality switch failed: Invalid representation");
        return false;
      }

      if (newIdx === videoQualityIdxRef.current) {
        console.log("Quality switch skipped: Already at target quality");
        return true;
      }
