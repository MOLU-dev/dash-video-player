...
export function useGrpcPlayer({
  ...
  const {
    ...
    onManifestUpdate: (manifest) => {
      console.log("[LIVE] Manifest updated", manifest);
    },

    setVideoReps,
    setAudioReps,

    // NEW: Handle stream end
    onStreamEnded: () => {
      console.log("[LIVE] 🔴 Stream ended - stopping playback");

      // Mark both streams as finished
      videoFinishedRef.current = true;
      audioFinishedRef.current = true;

      // Try to end the MediaSource stream
      tryEndStream();
      ...
      // Optional: Show a "Stream Ended" message
      // You can add a toast or overlay here
      console.log("✅ Live stream playback stopped");
    },
  });
