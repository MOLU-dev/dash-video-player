...
import { getSegmentCache } from "../../../src/lib/segmentCache";
...
interface UseSegmentFetchingProps {
  ...
  generatorRef: React.RefObject<"gpac" | "ffmpeg" | "unknown">; // Add this
}
...
export function useSegmentFetching({
  ...
  const fetchAndAppend = useCallback(
    async (
      ...
      return new Promise((resolve, reject) => {
        ...
        if (cachedData) {
          //  console.log(
          //    `[CACHE HIT] ${mediaType} segment ${segmentNumber} for rep ${rep.id}`
          //  );
          ...
          if (
            ...
            //  console.log(
            //    `Discarding cached segment ${segmentNumber} due to quality switch`
            //  );
          ...
          if (mediaType === "video") {
            updateThroughputMeasurement(cachedData.byteLength, 1, mediaType); // 1ms = instant
          }
          ...
          enqueueOperation(mediaType, () => {
            if (
              ...
              //  console.log(
              //    `Discarding cached segment ${segmentNumber} during append due to quality switch`
              //  );
            ...
            return appendBufferSafely(
              ...
              .then(() => {
                ...
                //  console.log(
                //    `Cached segment ${segmentNumber} appended successfully for rep ${rep.id}`
                //  );
              })
              .catch((error) => {
                if (error.name === "AbortError") {
                  //console.log(`Cached segment ${segmentNumber} append aborted`);
                } else {
                  //  console.error(
        ...
        //  console.log(
        //    `[CACHE MISS] Fetching ${mediaType} segment ${segmentNumber} from network`
        //  );
        ...
        const generator = generatorRef.current || "unknown";
        ...
        fetchSegment({
          ...
          .then((data) => {
            ...
            //  console.log(
            //    `[CACHED] ${mediaType} segment ${segmentNumber} for rep ${
            //      rep.id
    ...
    [
      videoId,
      segmentCache, // Add this dependency
      isOnlineRef,
      segmentOperationIdRef,
      segmentRequestIdRef,
      pendingSegmentOperationsRef,
  ...
  const fetchNextSegment = useCallback(
    async (
      ...
      // Stop if paused (unless it's a quality switch)
