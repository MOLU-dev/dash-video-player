...
export function usePlayerEffects({
  ...
  // OPTIMIZED: Auto mode quality selection with adaptive interval
  useEffect(() => {
    if (currentQuality === "auto") {
      const checkQuality = () => {
        // Skip if not visible or conditions aren't met
        if (
          !isVisibleRef.current ||
          isPausedRef.current ||
          (videoFinishedRef.current && audioFinishedRef.current) ||
          mediaSourceStateRef.current !== "open" ||
          !shouldAllowQualitySwitch("auto-mode")
        ) {
          qualityCheckTimeoutRef.current = window.setTimeout(
            checkQuality,
            3000
          );
          return;
        ...
        // Schedule next check
        qualityCheckTimeoutRef.current = window.setTimeout(checkQuality, 3000);
      };
