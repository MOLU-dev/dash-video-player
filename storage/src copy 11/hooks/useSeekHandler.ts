  ...
  const resetStreamForSeek = useCallback(
    async (time: number) => {
      ...
      try {
        ...
        // CRITICAL: Check if seek destination is unbuffered
        ...
        // Only skip clear if ALL conditions are met AND no problematic scenarios exist
        ...
        } else if (!isWithinKeepBehind) {
          clearReason = "outside keep-behind window";
        ...
        if (!shouldSkipClear) {
          ...
          console.log(
            `Preserving buffer for seek to ${time}s - Position is safely buffered within keep-behind window`
          );
        }
