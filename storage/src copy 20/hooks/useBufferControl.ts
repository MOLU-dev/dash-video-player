...
export function useBufferControl() {
  ...
  const calculateExponentialSlowdown = useCallback((
    ...
    // If stall is predicted, be more aggressive even at higher buffer levels
    ...
    // Normal slowdown calculation (as before)
    let delay = 0;
    let segmentsToFetch = 1;
    ...
    if (currentBuffer < BUFFER_EMERGENCY_THRESHOLD) {
      ...
      const progress = (currentBuffer - BUFFER_EMERGENCY_THRESHOLD) / (15 - BUFFER_EMERGENCY_THRESHOLD);
      delay = 50 + progress * 100;
      segmentsToFetch = mediaType === "video" ? 3 : 2;
    } else if (currentBuffer < targetBuffer * 0.5) {
      const progress = (currentBuffer - 15) / (targetBuffer * 0.5 - 15);
      const expFactor = Math.exp(-3 * progress);
      delay = 150 + expFactor * 850;
      segmentsToFetch = mediaType === "video" ? 2 : 1;
    } else if (currentBuffer < targetBuffer * 0.75) {
      const progress = (currentBuffer - targetBuffer * 0.5) / (targetBuffer * 0.75 - targetBuffer * 0.5);
      const expFactor = Math.exp(-3 * progress);
      delay = 1000 + expFactor * 2000;
      segmentsToFetch = 1;
    } else if (currentBuffer < targetBuffer) {
      const progress = (currentBuffer - targetBuffer * 0.75) / (targetBuffer - targetBuffer * 0.75);
      const expFactor = Math.exp(-3 * progress);
      delay = 3000 + expFactor * 4000;
      segmentsToFetch = 1;
    } else if (currentBuffer < maxBuffer) {
      const progress = (currentBuffer - targetBuffer) / (maxBuffer - targetBuffer);
      delay = 7000 + progress * 8000;
      segmentsToFetch = 1;
    } else {
      delay = 15000;
      segmentsToFetch = 0;
  ...
  const shouldFetchSegment = useCallback((
    ...
    if (combinedBufferAhead <= 0.1) {
      stallReason = "🚨 TRUE STALL: Buffer completely empty";
