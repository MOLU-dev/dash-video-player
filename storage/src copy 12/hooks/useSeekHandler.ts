  ...
  const resetStreamForSeek = useCallback(
    async (time: number) => {
      ...
      try {
        ...
        // ✅ USE VIDEO ELEMENT'S CURRENT TIME DIRECTLY
        ...
        console.log(
          `📍 Seek to ${time.toFixed(2)}s - Starting from segment ${
            videoNextSegRef.current
          }`
        );
        ...
        if (!shouldPreserve) {
          ...
          // ❌ REMOVED: No manual currentTime setting
        } else {
          // Buffer preserved - ensure adequate buffer ahead
