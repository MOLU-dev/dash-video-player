...
import { useVideoGestures } from "../../../src/hooks/useVideoGestures";
...
export function VideoPlayer({
  ...
  //     if (video) {
  //       const newTime = Math.min(video.duration, video.currentTime + amount);
  //       handleSeek(newTime);
  //       return amount;
  //     }
  //     return 0;
  ...
  //     const video = videoRef.current;
  //     if (video) {
  //       const newTime = Math.max(0, video.currentTime - amount);
  //       handleSeek(newTime);
  //       return -amount;
  //     }
  //     return 0;
  ...
  const {
    ...
    // onDoubleTapLeft: () => {
    //   const amount = seekBackward(); // applies the backward seek immediately (returns negative)
    //   // accumulate visual amount and show cumulative indicator
    //   accumulateSeek?.("left", amount);
    // },
    // onDoubleTapRight: () => {
    //   const amount = seekForward(); // applies the forward seek immediately (returns positive)
  ...
  //   const { type, value, side } = gestureIndicator;
  ...
  //   return (
  //     <div className={`gesture-indicator ${side || "center"}`}>
  //       <div className="gesture-icon">{getIcon()}</div>
  //       <div className="gesture-text">{getText()}</div>
  //     </div>
  //   );
  // };

  // Handle keyboard shortcuts (enhanced with gesture hook)

  // useEffect(() => {
  //   const handleKeyDown = (e: KeyboardEvent) => {
  //     // Ctrl+Shift+D (or Cmd+Shift+D on Mac) to clear all downloads
  //     if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === "D") {
  //       e.preventDefault();
  //       if (
  //         window.confirm(
  //           "Are you sure you want to clear ALL downloaded videos? This cannot be undone."
  //         )
  //       ) {
  //         clearAllDownloads();
  //       }
