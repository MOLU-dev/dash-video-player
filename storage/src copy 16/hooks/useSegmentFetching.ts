...
export function useSegmentFetching({
  ...
  const fetchNextSegment = useCallback(
    async (
      ...
      // ✅ LIVE STREAM: Check if segment is available
      if (isLive) {
        const range = getAvailableSegmentRange(
          rep.startNumber,
          rep.segmentDur,
          rep.timescale
        );

        if (nextSegRef.current > range.end) {
          console.log(
            `[LIVE] Waiting for segment ${nextSegRef.current}, current range: ${range.start}-${range.end}`
          );
          setTimeout(() => {
            if (!finishedRef.current) {
              fetchNextSegment(
        ...
        // For live, never mark as finished
      ...
      try {
        ...
        for (let i = 0; i < segmentsToFetch; i++) {
          ...
          // Check if we've reached the end (VOD only)
          if (!isLive && segNum > rep.startNumber + rep.totalSegments - 1) {
            finishedRef.current = true;
            break;
          }
