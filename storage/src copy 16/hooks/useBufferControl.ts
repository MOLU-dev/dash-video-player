...
//         // 5-10s: Fetch normally (emergency already handled <5s)
//         delay = 100;
//       } else if (actualBufferAhead < TARGET_BUFFER_LEVEL) {
//         // 10s to TARGET (60s): Gradual slowdown
//         const progress = Math.min(
//           1,
//           (actualBufferAhead - 10) / (TARGET_BUFFER_LEVEL - 10)
//         );
//         delay = 100 + progress * 900; // 100ms → 1000ms (1s)
//       } else if (actualBufferAhead < MAX_BUFFER_LEVEL) {
//         // TARGET (60s) to MAX (66s): Significant slowdown
//         const progress = Math.min(
//           1,
...
export function useBufferControl() {
  ...
  /**
   * Calculate if we should fetch and how long to wait
   * FIXED: Now fetches aggressively when buffer is low, slows down when buffer is high
   */
  const shouldFetchSegment = useCallback(
    (
      mediaType: "video" | "audio",
      ...
      // Emergency mode - always fetch immediately
      if (isEmergency || actualBufferAhead < BUFFER_EMERGENCY_THRESHOLD) {
        return {
          shouldFetch: true,
          delay: 0,
          reason: `Emergency: ${actualBufferAhead.toFixed(1)}s buffered`,
      ...
      // Calculate delay based on buffer level (faster when low, slower when high)
      let delay = 0;

      if (actualBufferAhead < 5) {
        // Very low buffer (0-5s): Fetch immediately
        delay = 0;
      } else if (actualBufferAhead < 10) {
        // Low buffer (5-10s): Small delay
        delay = 50;
      } else if (actualBufferAhead < TARGET_BUFFER_LEVEL) {
        // Building buffer (10-60s): Increasing delay as buffer fills
        const progress = Math.min(
          1,
          (actualBufferAhead - 10) / (TARGET_BUFFER_LEVEL - 10)
        );
        delay = 100 + progress * 1800; // 100ms → 1000ms (1s)
      } else if (actualBufferAhead < MAX_BUFFER_LEVEL) {
        // High buffer (60-66s): Slower fetching
        const progress = Math.min(
          1,
          (actualBufferAhead - TARGET_BUFFER_LEVEL) /
            (MAX_BUFFER_LEVEL - TARGET_BUFFER_LEVEL)
