...
export function CustomControls({
  ...
  const handleProgressClick = (e: React.MouseEvent<HTMLDivElement>) => {
    if (isLive) {
      return; // No seeking in live mode
    }
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    if (isLive) {
      return; // No dragging in live mode
    }
  ...
  const latency = isLive && liveEdge > 0 ? liveEdge - currentTime : 0;
  const isAtLiveEdge = latency < 5; // Within 5 seconds of live

  const handleMouseMove = (e: MouseEvent) => {
    if (!isDragging || !progressBarRef.current) return;

    setHasDragged(true); // Mark that user has actually dragged
  ...
  const handleMouseUp = () => {
    ...
    // Only seek if user actually dragged (not just a click)
    if (hasDragged) {
      onSeek(dragPreviewTime);
    }

    setIsDragging(false);
    setDragPreviewTime(0);
    // Don't reset hasDragged here - let the click handler do it
  };
  ...
  const handleTouchEnd = () => {
    ...
    // Only seek if user actually dragged (not just a tap)
    if (hasDragged) {
      onSeek(dragPreviewTime);
      setHasDragged(false); // Reset immediately for touch
    }
  ...
  }, [isDragging, duration, dragPreviewTime, hasDragged]);
  ...
  // Use drag preview time when dragging, otherwise use current time
  const displayProgressPercent = isDragging
    ? (dragPreviewTime / duration) * 100
    : progressPercent;
  ...
  //         {spriteUrl && vttUrl && (
  //           <ThumbnailPreview
  //             time={isDragging ? dragPreviewTime : hoverTime}
  //             spriteUrl={spriteUrl}
  //             vttUrl={vttUrl}
  ...
  return (
    ...
    >
      {/* Progress Bar Section */}
      <div
        className={`progress-container ${isLive ? "live-progress" : ""} ${
          isHoveringProgress || isDragging ? "hovering" : ""
        }`}
        ref={progressBarRef}
        onMouseEnter={() => !isLive && setIsHoveringProgress(true)}
        onMouseLeave={() => {
          if (!isDragging && !isLive) setIsHoveringProgress(false);
        }}
        onMouseMove={!isLive ? handleProgressHover : undefined}
        onClick={handleProgressClick}
        onMouseDown={handleMouseDown}
        onTouchStart={!isLive ? handleTouchStart : undefined}
      >
        {!isLive && spriteUrl && vttUrl && (
          <ThumbnailPreview
            time={isDragging ? dragPreviewTime : hoverTime}
            spriteUrl={spriteUrl}
            vttUrl={vttUrl}
        ...
        {isLive && (
          <div className="live-progress-bar-wrapper">
            ...
            <div className="live-status-text">
              ...
              <span>LIVE</span>
              {!isAtLiveEdge && latency > 0 && onSeekToLive && (
                <button className="go-live-button" onClick={onSeekToLive}>
                  Go to Live
                </button>
              )}
            </div>
      ...
      <div className="controls-bar">
        <div className="left-controls">
          ...
          {isLive && (
            <div className="live-time-badge">
              ...
              <span>LIVE</span>
              {!isAtLiveEdge && latency > 0 && (
                <span className="latency-text">-{Math.round(latency)}s</span>
              )}
            </div>
          )}
        </div>
        ...
        <div className="right-controls">
          ...
          {/* Theatre Mode - hide for live (optional) */}
          {!isFullscreen && (
            <button
              className="control-button"
              onClick={onTheatreModeToggle}
      ...
      <style jsx>{`
        ...
        .progress-container.live-progress {
          cursor: default;
          pointer-events: none;
          padding: 8px 12px;
          height: auto;
