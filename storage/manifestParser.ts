...
export function parseManifest(mpdXml: Uint8Array): ParsedManifest {
  ...
  // Get duration from MPD root element instead of Period
  const mpdElem = xmlDoc.querySelector("MPD");
  if (!mpdElem) {
    throw new Error("No <MPD> root element found");
  }

  // First try to get duration from mediaPresentationDuration attribute
  ...
  // Get all Period elements (though there's usually just one)
  ...
  for (const periodElem of periodElems) {
    ...
    for (const adaptSet of adaptationSets) {
      ...
      // Calculate total segments for this representation
      const totalSegments = Math.ceil(periodDuration / segmentDurationSec);
